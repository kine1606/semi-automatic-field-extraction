...
from app.services.classification.keyword_sets import (
    ENERGY_STRONG,
    ENERGY_WEAK,
    NAMEPLATE_STRONG,
    NAMEPLATE_WEAK,
...
from app.services.classification.pattern_rules import collect_nameplate_pattern_score
from app.services.classification.scoring import score_strong_phrases, score_weak_keywords
...
def _classify_from_text(
    ...
    energy_strong, energy_strong_hits = score_strong_phrases(normalized, ENERGY_STRONG)
    energy_weak, energy_weak_hits = score_weak_keywords(normalized, ENERGY_WEAK)

    nameplate_strong, nameplate_strong_hits = score_strong_phrases(normalized, NAMEPLATE_STRONG)
    nameplate_weak, nameplate_weak_hits = score_weak_keywords(normalized, NAMEPLATE_WEAK)
    nameplate_pattern_score, nameplate_pattern_hits = collect_nameplate_pattern_score(normalized)

    receipt_strong, receipt_hits = score_strong_phrases(normalized, RECEIPT_STRONG)
    ...
    signals = {
        ...
        "nameplate_pattern_score": nameplate_pattern_score,
    ...
    if include_debug:
        signals["matched"] = {
            ...
            "nameplate_patterns": nameplate_pattern_hits,
    ...
    is_nameplate = (
        (
            nameplate_strong >= 1
            and (nameplate_weak >= 1 or nameplate_pattern_score >= 2)
            and energy_strong == 0
            and green_ratio < 0.15
        )
        or (
            nameplate_pattern_score >= 3
            and energy_strong == 0
            and green_ratio < 0.15
        )
    )
    ...
    if is_nameplate:
        return _build_result(
            ...
            nameplate_confidence(
                nameplate_strong + min(nameplate_pattern_score, 2),
                nameplate_weak,
            ),
            (
                f"Nameplate strong={nameplate_strong}, "
                f"nameplate weak={nameplate_weak}, "
                f"pattern score={nameplate_pattern_score}"
            ),
            merged_text,
            signals,
            include_debug,
...
def classify_image(path: str, include_debug: bool = False) -> dict:
    ...
    if green_ratio < 0.05:
        quick_result = _classify_from_text(normalized, green_ratio, merged_text, include_debug, t0)
        if quick_result["imageType"] in {"raw_equipment_photo", "unknown", "nameplate_label"}:
            return quick_result
